...
#include "vroom/include/rapidjson/stringbuffer.h"
#include "vroom/include/rapidjson/writer.h"

namespace py = pybind11;

std::string solution_to_json_string(const vroom::Solution &sol, bool geometry) {
  auto json_output = vroom::io::to_json(sol, geometry);
...
void init_solution(py::module_ &m) {
  ...
  py::class_<vroom::Solution>(m, "Solution")
      ...
      .def("_to_json", &solution_to_json_string,
           py::arg("geometry") = false,
           "Returns the solution as a JSON string.")
